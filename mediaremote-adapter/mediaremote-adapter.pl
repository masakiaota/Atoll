...
my $framework_override = delete $ENV{ATOLL_MEDIAREMOTE_EXECUTABLE};
if (defined $framework_override) {
  fail "Invalid framework descriptor" unless $framework_override eq "/dev/fd/0";
}
my $framework = $framework_override
  // File::Spec->catfile($framework_path, $framework_basename);
